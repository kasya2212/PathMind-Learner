...
export async function assembleTutorLearnerContext(
  ...
  const next = scoped.nodes.length ? recommendNext(scoped.nodes, scoped.edges, decayed) : null;
  ...
  if (next) {
    ...
    const prereqNames = prereqIds.map((id) => nameOf(id)).filter((n): n is string => Boolean(n));
    const prereqsReady =
  ...
  const domainLabel = domain === DOMAIN ? "Java Backend" : (goalText ?? goalName ?? "Custom goal");
  ...
  if (unstarted.length) lines.push(`Not started yet: ${unstarted.slice(0, 8).join(", ")}.`);
  if (fading.length)
  ...
  if (daysLeft !== null)
    constraintBits.push(daysLeft >= 0 ? `${daysLeft} days left` : "past their deadline");
  ...
  if (!hasAnyAssessment)
    lines.push(
      "They have not completed any calibration yet — treat their skill levels as unknown.",
    );
  ...
  if (hiddenGapNames.length)
    iLines.push(
      `Hidden prerequisite gaps (never properly assessed): ${hiddenGapNames.join(", ")}.`,
    );
 ...
 * transient statuses (429/5xx) are automatically retried.
 */
async function callTutorModel(
  ...
  const request = async (): Promise<string> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const detail = rawResponse.slice(0, 1000);
  ...
  return fetchWithGeminiRetry(request, "TUTOR");
}
