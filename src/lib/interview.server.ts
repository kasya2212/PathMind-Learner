...
async function callInterviewModel(
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        "The interviewer isn't available right now — please check the Gemini API configuration.",
      );
    }

    if (response.status === 429) {
      throw new Error("The Gemini free-tier limit was reached — please wait and try again.",
);
    }

    if (response.status === 400) {
      throw new Error("The interviewer had trouble processing that request — please try again.",
);
    }
    ...
    throw new Error("Something went wrong with the interview — please try again.",
);
  }
  ...
  const categories: InterviewEvaluationCategory[] = rawCats
    .map((c) => {
      ...
      return {
        ...
        label: (LABELS.has(labelRaw) ? labelRaw : "Developing") as InterviewEvaluationCategory["label"],
...
function rowToEvaluation(row: {
  ...
  return {
    ...
    categories: (Array.isArray(row.categories) ? row.categories : []) as InterviewEvaluationCategory[],
...
export async function startInterviewSessionImpl(
  ...
  if (activeRow && !isStale(activeRow.started_at)) {
    ...
    return { session: toSessionDTO(activeRow), turns: await loadTurns(supabase, activeRow.id), resumed: true };
  }
...
export async function submitInterviewAnswerImpl(
  ...
  const reuseExisting = last?.role === "learner" && last.content === text && isRecent(last.created_at);
  if (!reuseExisting) {
    const { error: insertError } = await supabase.from("interview_turns").insert({
      session_id: sessionId,
      role: "learner",
...
export async function completeInterviewSessionImpl(
  ...
  if (row.status === "abandoned") throw new Error("This interview was ended without an evaluation.");
