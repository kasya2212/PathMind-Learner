...
function TutorPage() {
  ...
  async function handleSend(raw: string, opts?: { voice?: boolean }) {
    ...
    const now = Date.now();
    if (lastSentRef.current && lastSentRef.current.text === text && now - lastSentRef.current.at < 3000) {
      return;
    }
    ...
    >
      ...
      <div className="grid grid-cols-[minmax(0,1fr)] items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <Card className="flex min-w-0 flex-col overflow-hidden">
          ...
          {/* messages */}
          <div ref={scrollRef} className="h-[46dvh] min-h-72 overflow-y-auto px-4 py-4 lg:h-[calc(100dvh-21rem)]">
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center justify-between px-1 text-[11px]">
                    ...
                    <span className={cn(overLimit ? "font-medium text-destructive" : "text-muted-foreground")}>
                ...
                <Button type="submit" variant="primary" disabled={sending || !draft.trim() || overLimit}>
                  Send
                </Button>
              </form>
            )}
  ...
  return (
    ...
    >
      ...
      <div className={cn("break-words", isUser && "whitespace-pre-wrap")}>
        {isUser ? message.content : <ReactMarkdown components={mdComponents}>{message.content}</ReactMarkdown>}
      </div>
    </li>
  );
}
  ...
  return (
    <Card className="p-4">
      ...
      {contextQuery.isPending ? (
        ...
        <ErrorState
          message="Couldn't load your path."
